# مخططات تدفق البيانات للفصل الثالث

يحتوي هذا الملف على مخططات تدفق البيانات المقترحة بصيغة `Mermaid`، ويمكن نسخ كل مخطط وتحويله إلى صورة لإدراجه في وثيقة المشروع.

## 1. المخطط البيئي

```mermaid
flowchart LR
    C["العميل"]
    E["موظف التأمين"]
    M["الإدارة"]
    X["جهات خارجية"]
    S(["0\nمنصة أمان لإدارة التأمينات"])

    C -->|"طلب تسجيل / طلب تأمين / رفع وثائق"| S
    S -->|"بيانات الوثيقة / حالة المطالبة / إشعارات"| C

    E -->|"مراجعة طلبات / اعتماد وثائق / معالجة مطالبات"| S
    S -->|"قوائم العمل / بيانات العملاء / سجل المتابعة"| E

    M -->|"سياسات / صلاحيات / طلب تقارير"| S
    S -->|"تقارير / مؤشرات أداء / سجل عمليات"| M

    X -->|"نتائج دفع / تقارير معاينة"| S
    S -->|"أوامر دفع / طلبات تحقق / تقارير دورية"| X
```

## 2. المخطط الصفري

```mermaid
flowchart TB
    C["العميل"]
    E["موظف التأمين"]
    M["الإدارة"]
    X["جهات خارجية"]

    P1(["1.0\nإدارة العملاء والمستخدمين"])
    P2(["2.0\nإدارة المنتجات والتسعير والاكتتاب"])
    P3(["3.0\nإدارة الوثائق التأمينية"])
    P4(["4.0\nإدارة المطالبات"])
    P5(["5.0\nإدارة الأقساط والمدفوعات"])
    P6(["6.0\nإدارة الإشعارات والتقارير والرقابة"])

    D1[("D1\nبيانات العملاء والحسابات")]
    D2[("D2\nمنتجات التأمين وقواعد التسعير")]
    D3[("D3\nالوثائق التأمينية")]
    D4[("D4\nالمطالبات والمرفقات")]
    D5[("D5\nالأقساط والمدفوعات")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    C -->|"بيانات التسجيل"| P1
    C -->|"طلب تأمين جديد"| P2
    C -->|"طلب تجديد أو تعديل وثيقة"| P3
    C -->|"تسجيل مطالبة"| P4
    C -->|"سداد قسط"| P5
    P1 -->|"تأكيد الحساب"| C
    P3 -->|"بيانات الوثيقة"| C
    P4 -->|"قرار المطالبة"| C
    P5 -->|"إيصال السداد"| C
    P6 -->|"إشعارات وتنبيهات"| C

    E -->|"بيانات موظف"| P1
    E -->|"مراجعة منتج وتسعير"| P2
    E -->|"استعراض وثائق"| P3
    E -->|"معالجة مطالبة"| P4
    E -->|"متابعة مدفوعات"| P5
    P1 -->|"صلاحيات وقوائم عمل"| E
    P2 -->|"نتيجة التسعير"| E
    P3 -->|"بيانات الوثيقة"| E
    P4 -->|"قائمة المطالبات"| E
    P6 -->|"تنبيهات وإشعارات"| E

    M -->|"إدارة أدوار ومستخدمين"| P1
    M -->|"اعتماد منتجات وسياسات تسعير"| P2
    M -->|"طلب تقارير"| P6
    P6 -->|"تقارير ومؤشرات أداء"| M

    X -->|"نتائج دفع إلكتروني"| P4
    X -->|"نتائج دفع إلكتروني"| P5
    P4 -->|"طلب معاينة أو تحقق"| X
    P5 -->|"أمر دفع إلكتروني"| X

    P1 -->|"قراءة وتحديث بيانات العملاء"| D1
    D1 -->|"بيانات العملاء"| P1
    P2 -->|"قراءة وتحديث منتجات التسعير"| D2
    D2 -->|"قواعد التسعير"| P2
    P2 -->|"تسجيل بيانات العميل المقدم"| D1
    P3 -->|"قراءة وإصدار وتحديث الوثائق"| D3
    D3 -->|"بيانات الوثائق"| P3
    P3 -->|"قراءة بيانات العميل"| D1
    P3 -->|"قراءة نوع التأمين والتسعير"| D2
    P4 -->|"تسجيل وتحديث المطالبات"| D4
    D4 -->|"بيانات المطالبات"| P4
    P4 -->|"التحقق من الوثيقة السارية"| D3
    P4 -->|"قراءة بيانات العميل"| D1
    P5 -->|"إنشاء وتحديث الأقساط والدفعات"| D5
    D5 -->|"بيانات الأقساط"| P5
    P5 -->|"قراءة بيانات الوثيقة"| D3
    P5 -->|"قراءة بيانات العميل"| D1
    P6 -->|"حفظ الإشعارات وسجل العمليات"| D6
    D6 -->|"سجل العمليات"| P6
    P6 -->|"قراءة بيانات العملاء"| D1
    P6 -->|"قراءة بيانات الوثائق"| D3
    P6 -->|"قراءة بيانات المطالبات"| D4
    P6 -->|"قراءة بيانات المدفوعات"| D5
```

## 3. مخطط المستوى الأول للعملية 1.0 إدارة العملاء والمستخدمين

```mermaid
flowchart LR
    C["العميل"]
    E["الموظف"]
    D1[("D1\nبيانات العملاء والحسابات")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    P11(["1.1\nإنشاء حساب"])
    P12(["1.2\nالتحقق من البيانات"])
    P13(["1.3\nتحديث الملف الشخصي"])
    P14(["1.4\nإدارة الأدوار والصلاحيات"])

    C -->|"بيانات التسجيل"| P11
    E -->|"بيانات الموظف الجديد"| P11
    P11 -->|"بيانات الحساب الجديد"| P12
    P12 -->|"تأكيد الحساب أو رفضه"| C
    P12 -->|"حفظ بيانات موثقة"| D1
    E -->|"طلب تعديل بيانات"| P13
    D1 -->|"بيانات الملف الحالية"| P13
    P13 -->|"بيانات محدثة"| D1
    P13 -->|"إشعار التحديث"| C
    E -->|"تعيين دور أو صلاحية"| P14
    P11 -->|"تسجيل عملية إنشاء"| D6
    P12 -->|"تسجيل نتيجة التحقق"| D6
    P13 -->|"تسجيل عملية التعديل"| D6
    P14 -->|"تسجيل تغيير الصلاحية"| D6
```

## 4. مخطط المستوى الأول للعملية 2.0 و 3.0 إدارة الاكتتاب والوثائق

```mermaid
flowchart LR
    C["العميل"]
    U["موظف الاكتتاب"]
    D1[("D1\nبيانات العملاء")]
    D2[("D2\nالمنتجات وقواعد التسعير")]
    D3[("D3\nالوثائق التأمينية")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    P21(["2.1\nاستقبال طلب التأمين"])
    P22(["2.2\nاحتساب السعر المبدئي"])
    P23(["2.3\nمراجعة الاكتتاب"])
    P31(["3.1\nإصدار الوثيقة"])
    P32(["3.2\nتجديد أو تعديل أو إلغاء الوثيقة"])

    C -->|"بيانات الطلب ومرفقاته"| P21
    P21 -->|"حفظ بيانات العميل المقدم"| D1
    P21 -->|"قراءة منتج التأمين"| D2
    D2 -->|"قواعد التسعير وحدود التغطية"| P22
    P22 -->|"العرض السعري المبدئي"| U
    U -->|"قرار الاكتتاب"| P23
    P23 -->|"طلب معتمد"| P31
    P23 -->|"نتيجة الاكتتاب للعميل"| C
    P31 -->|"حفظ بيانات الوثيقة"| D3
    P31 -->|"إشعار إصدار الوثيقة"| C
    U -->|"طلب تجديد أو تعديل أو إلغاء"| P32
    D3 -->|"بيانات الوثيقة الحالية"| P32
    P32 -->|"تحديث سجل الوثيقة"| D3
    P21 -->|"تسجيل استقبال الطلب"| D6
    P22 -->|"تسجيل عملية التسعير"| D6
    P23 -->|"تسجيل قرار الاكتتاب"| D6
    P31 -->|"تسجيل إصدار الوثيقة"| D6
    P32 -->|"تسجيل تعديل الوثيقة"| D6
```

## 5. مخطط المستوى الأول للعملية 4.0 إدارة المطالبات

```mermaid
flowchart LR
    C["العميل"]
    E["موظف المطالبات"]
    X["خبير معاينة أو جهة خارجية"]
    D3[("D3\nالوثائق التأمينية")]
    D4[("D4\nالمطالبات والمرفقات")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    P41(["4.1\nتسجيل المطالبة"])
    P42(["4.2\nالتحقق من التغطية"])
    P43(["4.3\nاستكمال المستندات"])
    P44(["4.4\nالتقييم واتخاذ القرار"])
    P45(["4.5\nالاعتراض وإعادة الفحص"])

    C -->|"بيانات الحادث والمستندات"| P41
    P41 -->|"حفظ بيانات المطالبة الجديدة"| D4
    D3 -->|"بيانات الوثيقة السارية"| P42
    D4 -->|"بيانات المطالبة المسجلة"| P42
    E -->|"مراجعة شروط التغطية"| P42
    P42 -->|"طلب استكمال مستندات"| P43
    C -->|"رفع مستندات إضافية"| P43
    P43 -->|"تحديث ملف المطالبة"| D4
    P43 -->|"طلب معاينة خارجية"| X
    X -->|"تقرير المعاينة"| P44
    E -->|"قرار الاعتماد أو الرفض"| P44
    P44 -->|"تحديث حالة المطالبة"| D4
    P44 -->|"إشعار القرار للعميل"| C
    C -->|"طلب اعتراض على القرار"| P45
    P45 -->|"إحالة الاعتراض للمراجعة"| E
    P45 -->|"تسجيل الاعتراض"| D4
    P41 -->|"تسجيل فتح المطالبة"| D6
    P42 -->|"تسجيل نتيجة التحقق"| D6
    P43 -->|"تسجيل استكمال المستندات"| D6
    P44 -->|"تسجيل قرار المطالبة"| D6
    P45 -->|"تسجيل عملية الاعتراض"| D6
```

## 6. مخطط المستوى الأول للعملية 5.0 إدارة الأقساط والمدفوعات

```mermaid
flowchart LR
    C["العميل"]
    A["المحاسب"]
    X["بوابة دفع أو بنك"]
    D3[("D3\nالوثائق التأمينية")]
    D5[("D5\nالأقساط والمدفوعات")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    P51(["5.1\nإنشاء جدول الأقساط"])
    P52(["5.2\nاستقبال السداد"])
    P53(["5.3\nتأكيد الدفع وإصدار الإيصال"])
    P54(["5.4\nمتابعة المتأخرات"])

    D3 -->|"بيانات الوثيقة المصدرة"| P51
    P51 -->|"حفظ جدول الأقساط"| D5
    C -->|"طلب سداد قسط"| P52
    A -->|"تأكيد دفعة واردة"| P52
    P52 -->|"أمر دفع إلكتروني"| X
    X -->|"نتيجة الدفع الإلكتروني"| P53
    P53 -->|"تحديث حالة القسط"| D5
    P53 -->|"إيصال السداد للعميل"| C
    D5 -->|"بيانات الأقساط المتأخرة"| P54
    P54 -->|"تسجيل تنبيه التأخر"| D6
    P54 -->|"إشعار تحذير للعميل"| C
    P51 -->|"تسجيل إنشاء الجدول"| D6
    P52 -->|"تسجيل استقبال الدفعة"| D6
    P53 -->|"تسجيل إصدار الإيصال"| D6
```

## 7. مخطط المستوى الأول للعملية 6.0 الإشعارات والتقارير والرقابة

```mermaid
flowchart LR
    M["الإدارة"]
    E["الموظف"]
    C["العميل"]
    D1[("D1\nبيانات العملاء")]
    D3[("D3\nالوثائق")]
    D4[("D4\nالمطالبات")]
    D5[("D5\nالمدفوعات")]
    D6[("D6\nالإشعارات وسجل العمليات")]

    P61(["6.1\nتوليد الإشعارات"])
    P62(["6.2\nإعداد التقارير"])
    P63(["6.3\nتسجيل ومراجعة العمليات"])

    D1 -->|"بيانات العميل لتوليد الإشعار"| P61
    D3 -->|"حالة الوثيقة وتواريخ الانتهاء"| P61
    D4 -->|"تغييرات حالة المطالبة"| P61
    D5 -->|"استحقاقات الأقساط المتأخرة"| P61
    P61 -->|"حفظ سجل الإشعارات"| D6
    P61 -->|"إشعار للعميل"| C
    P61 -->|"تنبيه للموظف"| E

    M -->|"طلب تقرير مخصص"| P62
    D1 -->|"بيانات العملاء والحسابات"| P62
    D3 -->|"بيانات الوثائق السارية"| P62
    D4 -->|"بيانات المطالبات والحالات"| P62
    D5 -->|"بيانات التحصيل والمتأخرات"| P62
    P62 -->|"تقرير جاهز للإدارة"| M

    E -->|"طلب مراجعة سجل عملية"| P63
    M -->|"طلب تدقيق أو مراجعة"| P63
    P63 -->|"تسجيل عمليات الرقابة"| D6
    D6 -->|"سجل العمليات للمراجعة"| P63
```
